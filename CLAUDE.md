...
- **State** (`utils/state.py`): Pydantic models (`ChangedFile`, `ReviewUnit`, `Finding`, `ReviewResult`, `ReviewTaskState`, `SkillRef`, `AgentState`). The `review` node's **input schema is `ReviewTaskState`** (carries one `unit`); the fan-out edge issues `Send("review", ReviewTaskState(unit=u))` and the node returns `{"findings": …}` merged into `AgentState.findings` via an `Annotated[list[Finding], add]` reducer. `ReviewResult` is the LLM structured-output wrapper. No untyped dicts across module boundaries.
- **Detection** (`utils/detect.py`): extension map + shebang/first-line fallback + special filenames/paths (`Dockerfile`, `Dockerfile.*`, `*.Dockerfile`, direct `.github/workflows/*.yml|*.yaml`, root `.gitlab-ci.yml`, `Jenkinsfile`) → skill key. Files with no detector signal are unclassified and omitted from review units until a detector mapping and matching skill are added; the missing-skill hard-fail applies only after detection classifies a file as a programming-language key.
